    ...
    ILogger<EodAutomationOrchestratorService> logger) : IEodAutomationOrchestratorService
{
    ...
    private async Task PollForCompletionAsync(AutomationRunLog log, TimeZoneInfo tz, DateTime pollDeadlineUtc, CancellationToken ct)
    {
        while (true)
        {
            ...
            {
                // App shutting down mid-run: mark NotObserved (not NotEligible) if the window had
                // already been reached, since our observation was genuinely cut short.
    ...
    /// by design, so its absence after the window fully closed is a genuine problem.</summary>
    private static string DeriveOverallStatus(AutomationRunLog log)
    {
